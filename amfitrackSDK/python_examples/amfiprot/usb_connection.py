...
class USBConnection(Connection):
    ...
    def __init__(self, vendor_id: int, product_id: int, serial_number: str = None, read_timeout: int = None):
        ...
        self.connection_state = mp.Value('i', ConnectionState.INIT)
    ...
    def is_connected(self):
        return (self.connection_state.value == ConnectionState.CONNECTED)
    ...
    def start(self):
        ...
        if "linux" in sys.platform.lower() or "darwin" in sys.platform.lower():
            print("Linux/Mac")
            out_conn, in_conn = mp.Pipe()     # Used by sub task to signal to main that it is ready 
            self.usb_task = mp.Process(target=usb_task, args=(out_conn, usb_device_hash, self.connection_state, tx_ids, rx_queues, self.transmit_queue, self.global_receive_queue, self.node_update_queue, self.read_timeout))
            self.usb_task.start()
            in_conn.recv()  # Will block until something is received from usb_task
        else:
            ...
            self.usb_task_read = mp.Process(target=usb_task_read, args=(out_conn_read, usb_device_hash, self.connection_state, tx_ids, rx_queues, self.global_receive_queue, self.node_update_queue))
...
def usb_task(conn, usb_device_hash, state, tx_ids, rx_queues: List[mp.Queue], tx_queue: mp.Queue, global_receive_queue: mp.Queue, node_update_queue: mp.Queue, read_time_out: int = None):
    IN_ENDPOINT = 0x81
    OUT_ENDPOINT = 0x01
    RETRY_LIMIT = 10
    ...
    if read_time_out is None:
        read_time_out = 25
    elif read_time_out < 1 or read_time_out > 100:
        raise Exception("read_timeout must be between in the range [1:100]")
    ...
    conn.send(0)    # Notify main process that usb_task_dual is started

    while True:
        if state.value == ConnectionState.CONNECTED:
            # Send next pending packet if any left in queue
            if packet_to_send is not None:
                byte_data = array.array('B', [1])
                byte_data.extend(packet_to_send.to_bytes())
                byte_data.extend([0] * (64 - len(byte_data)))
                try:
                    bytes_written = dev.write(OUT_ENDPOINT, byte_data, timeout=1000)
                    packet_to_send = None
                except usb.core.USBError as e:
                    print("Task USB connection lost.")
                    usb.util.dispose_resources(dev)
                    del dev
                    state.value = ConnectionState.DISCONNECTED
                    continue
            elif not tx_queue.empty():
                packet_to_send = tx_queue.get_nowait()
            ...
            # Try to receive
            try:
                rx_data = dev.read(IN_ENDPOINT, USB_HID_REPORT_LENGTH, timeout = read_time_out)
        ...
        elif state.value == ConnectionState.DISCONNECTED:
            print("Task USB Reconnecting...")

            dev = get_usb_device_by_hash(usb_device_hash)

            if dev is not None:
                print("Task USB Connection re-established!")
                state.value = ConnectionState.CONNECTED
            else:
                time.sleep(1)
        else:
...
def usb_task_read(conn, usb_device_hash, state, tx_ids, rx_queues: List[mp.Queue], global_receive_queue: mp.Queue, node_update_queue: mp.Queue):
    IN_ENDPOINT = 0x81
    OUT_ENDPOINT = 0x01
    RETRY_LIMIT = 10
    ...
    while True:
        if state.value == ConnectionState.CONNECTED:
            ...
            try:
                ...
                print("Read Task USB connection lost.")
                usb.util.dispose_resources(dev)
                del dev
                state.value = ConnectionState.DISCONNECTED
                continue
            ...
            print("Read Task Reconnecting...")

            dev = get_usb_device_by_hash(usb_device_hash)

            if dev is not None:
                print("Read Task Connection re-established!")
                state.value = ConnectionState.CONNECTED
            else:
                time.sleep(1)
        else:
        ...
        if state == ConnectionState.CONNECTED:
            ...
            if packet_to_send is not None:
                ...
                try:
                    ...
                    print("Write Task USB connection lost.")
                    usb.util.dispose_resources(dev)
                    del dev
            ...
            print("Write Task Reconnecting...")

            dev = get_usb_device_by_hash(usb_device_hash)

            if dev is not None:
                print("Write Task Connection re-established!")
                state = ConnectionState.CONNECTED
            else:
                time.sleep(1)
        else:

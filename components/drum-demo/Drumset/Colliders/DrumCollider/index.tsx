...
export default function DrumCollider({
  ...
  const levaValues = useControls(
    name,
    {
      ...
      rx: { value: propRx, min: -Math.PI / 3, max: Math.PI / 3, step: 0.001, label: "Rotation X" },
      rz: { value: propRz, min: -Math.PI / 3, max: Math.PI / 3, step: 0.001, label: "Rotation Z" },
      bodyRadius: { value: propBodyRadius, min: 0.5, max: 4, step: 0.001, label: "Radius" },
      bodyHeight: { value: propBodyHeight, min: 0.01, max: 5, step: 0.001, label: "Height" },
      logValues: button(() => {
        console.log(`[DrumCollider "${name}" values]`, currentDebugValuesRef.current);
      }),
    },
    { collapsed: true },
  );
  ...
  currentDebugValuesRef.current = { px, py, pz, rx, rz, position, rotation, bodyRadius, bodyHeight };

  const isDebug = useDrumDemoStore((s) => s.isDebug);
  const [ref] = useCylinder(
    () => ({ type: "Static", args: [bodyRadius, bodyRadius, bodyHeight, 16], position, rotation }),
    undefined,
